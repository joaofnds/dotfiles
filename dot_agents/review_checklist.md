...
**This is a reference, not an instruction.** Nothing loads it. No skill, agent, or rule
file points here, and no reviewer reads it at review time — the rules do the work, as they
did before this file existed. Its only job is to answer two questions at a glance: what
are we checking, and what are we not. Add, cut, and reword lines freely; nothing
downstream breaks.
...
mandate enumerates exactly this ownership ("naming, comment policy, type-system escape
hatches, unparsed boundary input, error translation, entity construction and constructor/DI
shape, language-file idioms, hand-edits to generated files"). Every individual line below
...
- Does an entity hold behavior, or is it getters and setters with the logic in a service? *(`coding_style.md` §2a)*
...
- Is a state transition a named method returning a new instance, rather than in-place mutation or a service assembling props? *(`coding_style_typescript.md` §3)*
...
- Do constructors return concrete types while consumers declare the narrow interface they need? *(`coding_style_go.md` §6)*
...
- Is there an anti-corruption layer where an external model crosses in? *(`coding_style.md` §2d, `engineering_judgment.md` §2)*
...
- † Do full-object assertions use structural equality, escaping to a containment matcher for fields that are "volatile **or irrelevant**"? §5.1 is headed "**Prefer** structural equality" and says deep-equal "when you can". *(`03-test-aesthetics.md` §5.1)* — note `testing/00-index.md` §113 states the escape as "only for volatile fields", dropping the irrelevance half; §5.1 is the more specific file and governs. The false positive this creates is in §G.
...
- Evidence surviving a revert of the patch makes the finding advisory. A two-line edit inside a pre-existing 300-line function did not introduce Long Function. *(`panel-review` §3)*
