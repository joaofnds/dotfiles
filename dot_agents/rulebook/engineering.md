...
- **Find the box.** Sort the constraints into the ones something outside the session backs, what was asked for or a measurement of the need, and the ones someone assumed. A problem reads as impossible when the two are mixed. Sort again whenever the chosen path costs something nothing asked for: a capability, an edge case to handle, a dependency, or the conclusion that the problem cannot be solved with what you have. At that moment list every path, the one where the problem does not exist first (delete the thing, halt instead of handling, drop the assumed constraint, let the platform do it), and say for each what rules it out and how you know. The path that removes the problem wins unless something you can show rules it out. Write the list on the card, and in the commit body give the reader the paths that would otherwise be tried again, with what ruled each out, since a reader of the commit cannot open the card. When the winner changes what the task delivers, report it as the pick, with the list, and end the turn there. *(See: Pragmatic Programmer, Solving Impossible Puzzles)*
...
- **Seek the simplest thing that could work.** If two approaches solve it and one is a net deletion, that one wins. Simple is Beck's four criteria in order, `coding-style/core.md` under Core Philosophy, where fewest elements ranks fourth. *(See: simplicity-vs-ease, Kent Beck / XP)*
...
- **Related writes fail together or not at all.** When a requirement says two or more writes hold together as one logical change, a caller or a later read could observe one without the other and be wrong, no failure between them may leave that state visible. First ask whether one store can cover them, and use its transaction if so. Where it can't, a second store, a cache, an external call, an in-request retry loop is not enough: it loses the pending write's intent when the process dies, which is the failure this rule is about. Record the pending second write durably alongside the first and have something outside the request drive it to completion, within an explicit attempt or time budget, or add a step that undoes the first write. A boundary the design deliberately leaves eventually consistent is not this defect.
...
- **Stale verification.** A cached pass is not a run. When the point of the run is that
  something changed, confirm the runner actually rebuilt and re-ran: Go prints `(cached)` on
  a cached result, so bust it with `go test -count=1`; cargo prints a `Compiling` line, and if
  it does not, `touch` the changed source and run again; vitest caches transforms, not results,
  so a green run did execute the tests; when the suite imports built output rather than
  sources, rebuild that first, otherwise a mutation that survives is the test's gap, not the
  cache's. A suite that stays green after a mutation you expected to break it is a cache report
  until you have that evidence, never a fact about the tests.
