...
Stack-specific conventions for building UI. Read the generic `core.md` and `typescript.md` first; this file is the UI layer on top of them. Framework-agnostic by default: primary stack is **Next.js + Tailwind**, secondary is **Tauri + Svelte**. When a rule is framework-specific it says so.
