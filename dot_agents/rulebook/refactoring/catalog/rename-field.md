...
- `engineering.md`: name things in the domain's language: misaligned
  field names cause misaligned models, and this refactoring is the repair.
- `core.md`: domain models keep schemas and wire formats out: when the
  stored or serialized name must stay, the translation belongs at the boundary mapper,
  not as a permanently wrong domain name.
- `core.md`: surgical execution: rename the field the task establishes is
