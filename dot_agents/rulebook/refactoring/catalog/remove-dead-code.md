...
- `engineering.md`: code is a liability: dead code is the pure case, all
  maintenance cost and no behavior. When in doubt, delete.
- `engineering.md`: facts before theories: deadness is an empirical claim;
  the deletion is justified by the probe's result, never by memory of the call graph.
- `core.md`: surgical execution names leaving no dead imports behind from
