...
- **One switch, one place**: a single conditional over a closed set is clear, honest
  code; `core.md`'s "solely to satisfy this document" forbids conjuring a class hierarchy to satisfy a
...
- `core.md`: the hierarchy (or table) is an introduced element; a single
  non-repeated switch does not pay for it. The finding must show the repetition.
- `engineering.md`: program to interfaces, encapsulate what varies:
  this refactoring is that principle's canonical mechanical form.
- `core.md`: leverage the type system: in typed languages, prefer the
