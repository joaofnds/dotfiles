...
- **`core.md`'s "solely to satisfy this document" outranks the book here**: an immutable record passed around as
...
- `core.md`: do not introduce classes solely to satisfy a document: the
  class must be bought by demonstrated update paths and invariants, per "When not to
  apply."
- `core.md`: behavior lives with data: the anemic-model rule is why the
