...
- `core.md`: the house default is no class without demonstrated need; this
  refactoring is that rule executing in reverse on a class that lost (or never had)
  its justification.
- `engineering.md`: YAGNI: unused lifecycle hooks are the flexibility
  built for a future that never came; deleting them is the rule's enforcement arm.
- `engineering.md`: code is a liability: constructor, fields, and
