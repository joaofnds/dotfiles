...
- **The default is a function**: `core.md`'s "solely to satisfy this document" bars introducing a class to
...
- `core.md`: do not introduce classes solely to satisfy structure: the
  command must be evidenced by failed extraction attempts or a required lifecycle,
  never by a preference for objects.
- `engineering.md`: match complexity to the problem: the command sits at
