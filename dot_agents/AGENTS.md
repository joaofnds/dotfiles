...
- **Multi-stage feature, debug, review, or delivery work** → no rule file. Use only the stages task size justifies. The chain skills (`/discuss`, `/research`, `/grill`, `/plan`, `/build`), `/handoff`, `/art-direction`, `/absorb`, `/dream`, and `/kaizen` cannot be reached by model invocation (made user-invoked 2026-08-12 to cut dispatch context) — recommend the next stage to the user by name, using each skill's own description and skip conditions
