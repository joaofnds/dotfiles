...
The stage column is Research, the card type `bug`. A defect picked up from a legacy queue
file (a `.boris/BACKLOG.md` item): create the bug card from the item, move its evidence into
an attached doc, and delete the item's lines; the file dies with its last item. `backlog-board.md` §Migrate on touch
