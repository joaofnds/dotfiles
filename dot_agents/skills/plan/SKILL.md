...
7. **Testing strategy** — the exact command to run the tests (e.g. `pnpm test --run`, `go test ./...`), what to verify manually if tests aren't enough, and which existing tests are likely to break and why. When a spec exists, map every acceptance criterion to a runnable check or a named manual verification — an unmapped criterion is a plan gap, not a detail for `/build` to discover. Where that check is the project's own suite, write its boundary on the same line: which behavior, removed, must turn the suite red. "The suite passes" is satisfiable by deleting the failing test; a suite that stays green with the behavior gone is what `/build` has to be able to catch. For UI work with a ratified direction, map that file's **Verify** checks as a named manual verification.
