...
3. **Work the task tracker top to bottom, in order.** Do one item at a time, executing it via the loop in step 4, then flip it to `- [x]` in the plan file and commit before moving on — so progress survives if this session also dies. One commit per green task, staging only that task's files, subject in lowercase imperative. Invoking this skill is the user's standing direction to make these per-task commits — don't re-ask for each one. Each green task is then a checkpoint: a later failure is diagnosed or reverted against the last green commit, not against the whole feature.
...
6. **Refactor pass on green.** Reread the diff and remove complexity, duplication, or poor naming introduced or exposed by this change when the present benefit is demonstrated. Do not add future-facing abstractions. Rerun affected tests, and commit the refactor pass as its own commit.
