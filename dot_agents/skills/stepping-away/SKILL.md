...
Write it when you stop. A returning user reads from the bottom of the screen up, so order the report by rising urgency: the decision log's file path, then improvement ideas that would widen scope, as suggestions, never into the work, then the queued asks and parked questions, each a one-line reply away from moving. Close with the verdict, done or blocked, as the last line, saying "nothing blocking" out loud when it is true; when the report carries findings, that line is also the closing sentence `~/.agents/rules/reporting-findings.md` §The closing sentence requires, not a line after it.
