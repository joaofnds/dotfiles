...
shrinks the open set, and reports only the calls that are João's. Every change goes
through the `backlog` CLI. A stale card sometimes makes a fix obvious. A small
reversible one is committed as it would be anywhere, and the card closes citing the
commit. Anything larger stays a card.
...
anything a card might already answer or depend on.

A board with no milestone has no recorded goal, and "no consequence for the goal"
is one of the closure reasons. On such a board, do everything except priorities and
the queue, and make the goal item one on the list to João.
...
needs) against the repository, by grep or `git log`. A stale premise gets a dated
note with the current fact beside the writer's evidence.
...
- **Overtaken**: a Done card resolved the same symptom under another title, a doc
  ...
  to catch work done and never closed. Close it with the citation: the Done card,
  the commit, or the grep result that overtakes it.
...
settles it; otherwise split it out.
...
note, or a reader named on a Review card is, since the holding session would want
it written.
...
Every closure cites one of: addressed by commit (the card closes as Done, the
commit in its final summary), superseded by another card, out of scope (João's
own answer), or no consequence for the board's goal. The last three archive with
the citation. A run must shrink the open set or say, on the triage doc, why it
could not.

A card João declines is recorded as a decision on the board (`backlog decision
create`), titled by the concept, with his reason, so the same idea doesn't return
as a new card next week.

A finding that recurs across cards and traces to how they're written, not to what
they're about, is not fixed one card at a time. It goes on the numbered list to
...
board. When the queue is older, say so and let him direct a run.

Nothing closes or merges without its citation, nothing in Build or Review changes
status or priority, and priority between features is João's.
