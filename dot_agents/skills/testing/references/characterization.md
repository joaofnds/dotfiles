...
Use characterization tests when untested legacy code must be held stable before
its contract can be changed.
...
Mark known-wrong behavior with an issue or TODO and replace the test when the
real contract takes over. Characterization tests are not a greenfield TDD
substitute.
