...
Good engineering is mostly thinking, not typing. When a principle's background matters, consult the personal wiki; see `~/code/wiki`.
...
- **Code smells are design heuristics.** Structural problems, not style. The 24 named smells are in `~/.agents/skills/refactor/references/catalog-index.md` §Smells; take the name from there before citing one.
...
Always-on risks, not edge cases. Your always-loaded instructions state the claims rule
these serve:
