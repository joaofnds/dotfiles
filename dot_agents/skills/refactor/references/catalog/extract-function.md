...
- `the style skill`: comments default to zero, and extraction is move 2 of the three
  moves that must be exhausted before writing one. A block you were about to comment is
  this refactoring's primary trigger.
- `the style skill`: Beck's ordering ranks fewest-elements fourth. Extraction adds
...
- `the style skill`: surgical execution: extract within the code the task touches;
