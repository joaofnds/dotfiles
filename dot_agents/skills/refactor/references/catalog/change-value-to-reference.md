...
- `the style skill`: inject side-effecting or replaceable dependencies: the
...
- `the doctrine skill`: DRY is about knowledge: N copies of one entity are N
  encodings of the same fact, and this refactoring is the unification.
- `the style skill`: Beck's ordering: the repository is an added element, bought
