...
- `the doctrine skill`: YAGNI: the empty layer is preserved speculation, and
  this refactoring is the cleanup crew.
- `the doctrine skill`: code is a liability: a class boundary is code too;
  when it separates nothing, it only charges comprehension rent.
- `the style skill`: Beck's ordering: pure fewest-elements win: no intent lives
