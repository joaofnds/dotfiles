...
- `the style skill`: Tell, Don't Ask: a function interrogating a neighbor's
...
- `the style skill`: put domain behavior with the model it governs; Move Function
  is the mechanical step that enforces it.
- `the doctrine skill`: dependencies point inward: verify the move does not
