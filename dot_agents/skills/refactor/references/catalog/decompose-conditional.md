...
- `the style skill`: comments default to zero: branch-labelling comments are this
  refactoring not yet performed; the extracted names replace them.
- `the style skill`: boring control flow: the `if` stays a plain `if`: the
  refactoring simplifies what it coordinates, not the control structure itself.
- `the style skill`: Beck's ordering: three named functions are added elements
