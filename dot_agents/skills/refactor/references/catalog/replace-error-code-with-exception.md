...
- `the style skill`: error translation at boundaries: the thrown type must be a
  stable application/port error; letting driver or HTTP errors fly raw just moves the
  problem.
- `the style skill`: "use domain errors only for domain outcomes": the same
  boundary discipline that governs which failures deserve an exception at all.
- `the doctrine skill`: narrows future bugs: the forgotten-check class
