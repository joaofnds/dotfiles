...
- **This house prefers composition** (`the doctrine skill` "Program to interfaces, encapsulate what varies"): when the shared
...
- `the doctrine skill`: composition over inheritance: the house default
  reverses Fowler's coin-flip; extraction of a parent must argue why a composed
  shared type serves worse.
- `the style skill`: the superclass is an introduced element: duplication between
