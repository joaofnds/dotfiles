...
- **The default is a function**: `the style skill`'s "solely to satisfy this document" bars introducing a class to
...
- `the style skill`: do not introduce classes solely to satisfy structure: the
  command must be evidenced by failed extraction attempts or a required lifecycle,
  never by a preference for objects.
- `the doctrine skill`: match complexity to the problem: the command sits at
