...
- `the doctrine skill`: dependencies point inward: the whole-object form is
...
- `the style skill`: generic utilities carve-out: `clamp`-style helpers take
  primitives by design; this refactoring's scope is domain functions, not utilities.
- `the style skill`: Tell, Don't Ask: caller-side unpacking is asking; the
