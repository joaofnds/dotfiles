...
- `the style skill`: don't defend against your own code: if *you* decide whether
...
- `the doctrine skill`: DRY over knowledge: the duplicated default ("occupant")
  is one business fact encoded N times; the object makes it one.
- `the style skill`: a class is not required: a frozen literal satisfies the
