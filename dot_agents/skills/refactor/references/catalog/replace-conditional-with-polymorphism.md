...
- **One switch, one place**: a single conditional over a closed set is clear, honest
  code; `the style skill`'s "solely to satisfy this document" forbids conjuring a class hierarchy to satisfy a
...
- `the style skill`: the hierarchy (or table) is an introduced element; a single
  non-repeated switch does not pay for it. The finding must show the repetition.
- `the doctrine skill`: program to interfaces, encapsulate what varies:
  this refactoring is that principle's canonical mechanical form.
- `the style skill`: leverage the type system: in typed languages, prefer the
