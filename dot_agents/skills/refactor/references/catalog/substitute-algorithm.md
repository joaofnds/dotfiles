...
- The behavior is not pinned by tests. A rewrite without a safety net is a gamble,
  ...
  (`~/.agents/skills/testing/references/characterization.md`).
...
- `the doctrine skill`: never program by coincidence: substituting an
  algorithm you haven't fully understood is that rule's named time bomb.
- `the doctrine skill`: prefer removing the cause over compensating: where
