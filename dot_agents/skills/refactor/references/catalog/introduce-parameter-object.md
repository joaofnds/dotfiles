...
- `the style skill`: a plain immutable record satisfies this refactoring in JS; a
  class enters only when behavior (like `contains`) accrues to justify it: the
  progression is earned, not front-loaded.
- `the style skill`: leverage the type system: in typed languages the named shape
  lets the compiler police what "two numbers" never could: mixed-up argument order
  dies at compile time.
- `the doctrine skill`: name things in the domain's language: the object's
