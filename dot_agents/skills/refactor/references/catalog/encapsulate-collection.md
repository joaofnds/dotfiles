...
- `the style skill`: behavior lives with data: scattered `push` calls on an
  exposed list are the anemic-model pattern in miniature; the add/remove methods bring
  the behavior home.
- `the style skill`: Tell, Don't Ask: `course.enroll(student)` tells;
  `course.roster.push(student)` asks for internals and decides for them.
- `the doctrine skill`: narrows future bugs: returning copies makes the
