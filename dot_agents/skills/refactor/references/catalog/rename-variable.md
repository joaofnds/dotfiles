...
- `the style skill`: move understanding from your head into the code: "renaming
  ... is how the persistence happens." This refactoring is that rule's primary verb.
- `the style skill`: comments default to zero, and a clearer name is move 1 of the
  three moves to exhaust before writing one; many comments are renames in disguise.
- `the style skill`: preserve established idioms: the rule that protects `i`, `e`,
