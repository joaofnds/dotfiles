...
- `the style skill`: explicit construction: entities map properties explicitly at
  creation; a post-construction setter phase is the bulk-assignment pattern this rule
  exists to prevent, one field at a time.
- `the style skill`: behavior lives with data: getter/setter pairs with logic
  elsewhere define the anemic model; every deleted setter moves the design away from
  it.
- `the doctrine skill`: narrows future bugs: the whole class of
