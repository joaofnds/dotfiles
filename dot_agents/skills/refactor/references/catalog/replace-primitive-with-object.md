...
- **`the style skill`'s "solely to satisfy this document" outranks the book**: a primitive that is only stored and
...
- `the style skill`: leverage the type system: an opaque string sniffed at each
  use is a runtime cast in disguise; parsing once into a real type at the boundary is
  the sanctioned shape.
- `the style skill`: the class must be earned by demonstrated duplicated behavior
  or invalid states, never introduced for taxonomy.
- `the style skill`: safe parsing at boundaries: the value object's validating
