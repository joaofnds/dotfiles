...
- `the style skill`: transformations are "stateless, non-mutating translators …
...
- `the style skill`: where this refactoring and Combine Functions into Class both
  fit, the transform wins by default: it delivers the single change site without
  introducing a class.
- `the doctrine skill`: complexity carries the burden of proof: the transform
