...
Stack-specific conventions for building UI. Read the generic `coding-style.md` and `coding-style-typescript.md` first; this file is the UI layer on top of them. Framework-agnostic by default: primary stack is **Next.js + Tailwind**, secondary is **Tauri + Svelte**. When a rule is framework-specific it says so.

Taste and aesthetic direction live in the `art-direction` skill, which is currently user-invoked only; check your available-skills context, and recommend it with a reason when it isn't there. This file's conventions are the floor under all UI work, ratified direction or not.
