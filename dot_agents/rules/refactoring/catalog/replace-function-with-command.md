...
- **The default is a function**: `coding-style.md`'s "solely to satisfy this document" bars introducing a class to
...
- `coding-style.md`: do not introduce classes solely to satisfy structure: the
  command must be evidenced by failed extraction attempts or a required lifecycle,
  never by a preference for objects.
- `engineering-judgment.md`: match complexity to the problem: the command sits at
