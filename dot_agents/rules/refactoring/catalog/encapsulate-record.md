...
- **`coding_style.md:8` outranks the book here**: an immutable record passed around as
...
- `coding_style.md:8` — do not introduce classes solely to satisfy a document: the
  class must be bought by demonstrated update paths and invariants, per "When not to
  apply."
- `coding_style.md:33` — behavior lives with data: the anemic-model rule is why the
