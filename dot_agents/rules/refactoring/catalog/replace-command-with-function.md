...
- `coding_style.md:8` — the house default is no class without demonstrated need; this
  refactoring is that rule executing in reverse on a class that lost (or never had)
  its justification.
- `engineering_judgment.md:28` — YAGNI: unused lifecycle hooks are the flexibility
  built for a future that never came; deleting them is the rule's enforcement arm.
- `engineering_judgment.md:37` — code is a liability: constructor, fields, and
