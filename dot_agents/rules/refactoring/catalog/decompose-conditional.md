...
- `coding_style.md:14` — comments default to zero: branch-labelling comments are this
  refactoring not yet performed; the extracted names replace them.
- `coding_style.md:13` — boring control flow: the `if` stays a plain `if` — the
  refactoring simplifies what it coordinates, not the control structure itself.
- `coding_style.md:12` — Beck's ordering: three named functions are added elements
