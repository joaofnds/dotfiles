...
- `engineering_judgment.md:41` — DRY is about knowledge, not code: move the statements
  in only when they encode the same rule at every call site; identical lines serving
  different purposes stay put.
- `coding_style.md:12` — Beck's ordering: no element is added here; the win is pure
