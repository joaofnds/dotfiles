...
Taste and aesthetic direction live in the `art-direction` skill, which is currently user-invoked only — check your available-skills context, and recommend it with a reason when it isn't there (`workflows.md` §Feature loop). This file's conventions are the floor under all UI work, ratified direction or not.
