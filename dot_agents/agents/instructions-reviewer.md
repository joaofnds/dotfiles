...
- **Standing artifact** — a path or file list (a new skill, an agent, a rules file, the
  ...
  `workflows.md` is a target when a caller names it: review its structure and citations, and never
  treat it as a source of obligation (its own header and `~/.agents/AGENTS.md` §Task lifecycle say
  the same).
...
**This file is mirrored into six consumers — edit in step, per §4 Deliberate mirror copies.** The severity ladder above is re-derived in `~/.agents/rules/reporting_findings.md` §Reading a reviewer's severity ladder and, through it, in `~/.agents/AGENTS.md` §Task lifecycle stop condition (1); §Output format's `Apply state` and `Outside this diff` classes are restated in that same section, straight from here; §1's per-file budgets in the dotfiles repo's `scripts/check-corpus-budgets.sh` and in `~/.agents/rules/instruction_external_facts.md` §1's standing list; the §Output format contract and the section numbering in `dot_agents/evals/instructions-reviewer/*/CASE.md`; the session-grounded launch contract in `~/.agents/skills/kaizen/SKILL.md` §Spawn the fresh critic. Every declaration today points inward, from those files to this one, so an edit made here sees no pointer back.
...
- Per-file budgets:
  ...
  - **`.claude/rules/` files**: with `paths:` frontmatter, the just-in-time budget, and that kind is not re-injected after compaction. Without it the file loads at launch at `.claude/CLAUDE.md` priority, so it takes the always-loaded router budget; whether *that* kind survives compaction is unrecorded (`instruction_external_facts.md` §1, 2026-08-06 second pass, which quotes only the `paths:` case).
  ...
  - **Hook-injected instruction text**: budgeted against the reach its event buys (Loading-path integrity below). A `UserPromptSubmit` injection lands on every main-thread prompt, so hold it to the always-loaded router budget; a `PostToolUse` injection fires per matching tool call, in subagents too, so budget it against the tool-call rate rather than the prompt count — a few lines, and a pointer at the rule rather than a restatement of one.
...
## Findings
…  (`No findings.` goes here, alone, when every severity below is empty)
