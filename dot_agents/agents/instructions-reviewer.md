...
**This file is mirrored into six consumers — edit in step, per §4 Deliberate mirror copies.** The severity ladder above is re-derived in `~/.agents/rules/reporting_findings.md` §Reading a reviewer's severity ladder and, through it, in `~/.agents/AGENTS.md` §Task lifecycle stop condition (1); §Output format's `Apply state` and `Outside this diff` classes are restated in that same section, straight from here; §1's per-file budgets and §3's "Compressible prose" bullet title in the dotfiles repo's `scripts/check-corpus-budgets.sh` and in `~/.agents/rules/instruction_external_facts.md` §1's standing list; the §Output format contract and the section numbering in `dot_agents/evals/instructions-reviewer/*/CASE.md`; the session-grounded launch contract in `~/.agents/skills/kaizen/SKILL.md` §Spawn the fresh critic. Every declaration today points inward, from those files to this one, so an edit made here sees no pointer back.
...
- **Compressible prose is a finding, and headroom never answers it.** When a shorter form keeps the meaning, conditions, and exceptions *and* drops a clause the reader processes on every load — restated context, choreography, a condition phrased twice — quote it, show the shorter form, rank it **Minor**. A saving that drops no clause is style-only and stays unreported; a house delta plus its one failure-mode clause is never compressible here (Operating notes, keep-side test). No budget answers one: every number in `scripts/check-corpus-budgets.sh` is a cap, never an allowance. (Added 2026-08-07, when the ceiling rose to 11,000 and the gap read as room to spend.)
